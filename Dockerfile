...
# --no-access-log: /v1/authorize already logs a structured decision
# line per request (main.py); /healthz and /v1/grants don't need one --
# Uvicorn's own plain-text access log would just be noise, dominated by
# healthz pings every few seconds.
CMD ["uvicorn", "services.authz.main:app", "--host", "0.0.0.0", "--port", "8080", "--no-access-log"]
